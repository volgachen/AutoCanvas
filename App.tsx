...
export default function AIWriterCanvas() {
  ...
  const [mode, setMode] = useState('code'); // 'code' | 'story'
  ...
  const callAI = async (userMessage) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { id: Date.now().toString(), role: 'assistant', content: `Error: ${error.message}. Please check your API Key.` }]);
    } finally {
      setIsLoading(false);
    }
  };
